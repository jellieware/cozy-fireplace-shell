...
#define MAX_W 512
#define MAX_H 256
#define MAX_SPARKS 1500  // Increased buffer to handle significantly more active sparks
...
void update_fire(int fire_w, int fire_h) {
    ...
    // 4. Higher density, fast-fading rising spark generator
    for (int y = 0; y < fire_h; y++) {
        for (int x = 0; x < fire_w; x++) {
            int heat = fire_pixels[y][x];
            
            // Increased spawn probability (1 in 40 per frame, ~10x more frequent)
            if (heat >= 4 && heat <= 15 && (rand() % 40 == 0)) {
                for (int i = 0; i < MAX_SPARKS; i++) {
                    if (sparks[i].life <= 0) {
                        ...
                        // Reduced lifespan (5–12 frames instead of 15–40) so they fade rapidly
...
void render_scene(int fire_w, int fire_h) {
    ...
    // Render faster-fading spark overlay
    for (int i = 0; i < MAX_SPARKS; i++) {
        if (sparks[i].life > 0) {
